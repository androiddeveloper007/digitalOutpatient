...
public class NiceSpinner extends android.support.v7.widget.AppCompatEditText {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        textColor = typedArray.getColor(R.styleable.NiceSpinner_textTintColor, -1);
        setTextColor(textColor);
        hintText = typedArray.getString(R.styleable.NiceSpinner_textTint);
        setText(hintText);
        setGravity(Gravity.CENTER);//CENTER_VERTICAL
        if (Build.VERSION.SDK_INT > 15 ) {
//            setPaddingRelative(24, 0, 0,  0);
...
//            setPadding(24, 0, 0,  0);
    ...
    /**
     * Set the default_0 spinner item using its index
     * 
...
//        setText(adapter.getItemInDataset(selectedIndex).toString());
    ...
    public Runnable longclickrun = new Runnable() {
        @Override
        public void run() {
            longclicked=true;
            setEnabled(true);
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            if(longclicked){
                ...
                if(popupwindowClosed !=null){
                    popupwindowClosed.closed();
                }
            ...
            removeCallbacks(longclickrun);
        } else if(event.getAction() == MotionEvent.ACTION_DOWN){
            longclicked=false;
            setEnabled(false);
            postDelayed(longclickrun, ViewConfiguration.getLongPressTimeout());
        }
        return super.onTouchEvent(event);
    }
    ...
    public void setOnPopupwindowCloseListener(popupwindowClosed popupwindowClosed){
        this.popupwindowClosed = popupwindowClosed;
    }

    public interface popupwindowClosed{
        void closed();
    }

    private popupwindowClosed popupwindowClosed;
